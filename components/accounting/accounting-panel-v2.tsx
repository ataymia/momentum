...
const sourceTypes: SourceEventType[] = ["Invoice issued", "Payment cleared", "Credit applied", "Refund settled", "Inventory receipt", "Inventory delivered", "Inventory adjustment", "Expense paid"];
const journalTone = (status: string) => status === "Posted" ? "success" as const : status === "Voided" ? "danger" as const : "warning" as const;
const sourceControlledJournal = (sourceType: SourceEventType | "Manual") => ["Payment cleared", "Payment reversed", "Payroll released", "Payroll voided"].includes(sourceType);
...
export function AccountingPanel() {
  ...
  const run = () => {
    ...
    setNotice(`${count} accounting ${count === 1 ? "action" : "actions"} processed`);
  };

  return (
    <div className="page accounting-depth-panel">
      <Section title="Native accounting ledger" description="Operational source events become accounting work through approved effective-dated rules or source-controlled system logic. Policy-dependent inventory amounts remain blocked until valuation and cost rules are configured." action={notice ? <StatusPill tone="success">{notice}</StatusPill> : undefined}>
        <div className="company-rule-facts"><div><span>Accounting basis</span><strong>{accounting.settings.basis}</strong><small>Configure before treating reports as financial statements</small></div><div><span>Inventory valuation</span><strong>{accounting.settings.inventoryValuation}</strong><small>COGS remains blocked while not configured</small></div><div><span>Unprocessed events</span><strong>{pending.length}</strong><small>{automatable.length} ready · {blocked.length} blocked</small></div><div><span>Posted journal balance</span><strong>{Math.abs(debits - credits) < 0.005 ? "Balanced" : "Investigate"}</strong><small>{formatMoney(debits)} debit · {formatMoney(credits)} credit</small></div></div>
        <div className="account-detail__actions"><label className="reassign-field"><span>Basis</span><select value={accounting.settings.basis} onChange={(event) => setSettings(event.target.value as AccountingBasis, accounting.settings.inventoryValuation)}><option>Not configured</option><option>Cash</option><option>Accrual</option></select></label><label className="reassign-field"><span>Inventory valuation</span><select value={accounting.settings.inventoryValuation} onChange={(event) => setSettings(accounting.settings.basis, event.target.value as InventoryValuation)}><option>Not configured</option><option>FIFO</option><option>Weighted average</option></select></label><Button size="sm" icon={<RefreshCcw size={14} />} onClick={run}>Run accounting automation</Button></div>
      </Section>
      ...
      {tab === "inbox" && <Section title="Accounting source-event inbox" description="Each source event shows whether accounting can process it now or which approved policy or source control is still missing">
