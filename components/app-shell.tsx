...
export function AppShell() {
  ...
  const [searchOpen, setSearchOpen] = useState(false); const [notificationsOpen, setNotificationsOpen] = useState(false);
  const [userOpen, setUserOpen] = useState(false); const [query, setQuery] = useState("");

  useEffect(() => {
    const listener = (event: KeyboardEvent) => {
      if ((event.metaKey || event.ctrlKey) && event.key.toLowerCase() === "k") { event.preventDefault(); setSearchOpen(true); }
      if (event.key === "Escape") { setSearchOpen(false); setNotificationsOpen(false); setUserOpen(false); }
    };
    addEventListener("keydown", listener); return () => removeEventListener("keydown", listener);
  }, []);

  const allowedPrimary = currentUser ? primaryNav.filter(item => canAccessPage(currentUser, item.key)) : [];
  const allowedSecondary = currentUser ? secondaryNav.filter(item => canAccessPage(currentUser, item.key)) : [];
  ...
  const searchResults = useMemo(() => {
    ...
    const accounts = (currentUser && canAccessPage(currentUser, "accounts") ? scope.accounts : []).filter(account => [account.name,account.location,account.contactName,account.channel].join(" ").toLowerCase().includes(normalized)).slice(0,5).map(account => ({ id: account.id, type: "Account", title: account.name, detail: `${account.location} · ${account.stage}`, page: "accounts" as PageKey, focus: account.id }));
    const orders = scope.orders.filter(order => { const account = scope.accounts.find(item => item.id === order.accountId); return `${order.number} ${account?.name ?? ""}`.toLowerCase().includes(normalized); }).slice(0,4).map(order => ({ id: order.id, type: "Order", title: order.number, detail: `${order.cases} cases · ${order.status}`, page: "orders" as PageKey, focus: order.id }));
    return [...accounts, ...orders];
  }, [currentUser, query, scope.accounts, scope.orders]);
  if (!currentUser) return null;

  const allNav = [...primaryNav, ...secondaryNav]; const activeNav = allNav.find(item => item.key === activePage);
  const pageName = activeNav ? labelFor(activeNav, currentUser) : "Home"; const customerMode = currentUser.role === "Customer";
  ...
  return <div className={`app-layout ${sidebarCollapsed ? "is-sidebar-collapsed" : ""}`}>
    <aside className={`sidebar ${sidebarOpen ? "is-open" : ""} ${sidebarCollapsed ? "is-collapsed" : ""}`}>
      <div className="sidebar__top"><BrandMark />
        <button className="sidebar__collapse" onClick={() => setSidebarCollapsed(!sidebarCollapsed)} aria-label={sidebarCollapsed ? "Expand menu" : "Collapse menu"} title={sidebarCollapsed ? "Expand menu" : "Collapse menu"}>{sidebarCollapsed ? <PanelLeftOpen size={19}/> : <PanelLeftClose size={19}/>}</button>
    ...
    <div className="app-main">
      ...
      <div className="page-container"><PageView/></div>
    </div>
