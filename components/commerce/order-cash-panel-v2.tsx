...
import { InvoiceTerms, PaymentMethod, arAgingBucket, computedInvoiceStatus, invoiceBalance, invoicePaidAmount, invoicePendingAmount, invoiceRecordableAmount, openReceivables, paymentSettlementDate } from "../../lib/commerce-engine";
...
export function OrderCashPanel() {
  ...
  return (
    <div className="page commerce-depth-panel">
      <Section title="Order-to-cash control" description="Orders, invoices, payments, credits, quality-refund exceptions, and reconciliation remain separate records with one traceable chain.">
      ...
      <div className="company-grid company-grid--two">
        ...
        {selected && <Section title="Invoice detail" description="Customer, location, order, terms, settlement, credit, and notes">
          ...
          <div className="accounting-rule-list">
            {payments.map(({ allocation, payment }) => payment && <article key={allocation.id}><span><Landmark size={17} /></span><div><strong>{formatMoney(allocation.amount)} · {payment.method}</strong><p>{payment.processorReference ?? "No processor reference"} · allocated to {selected.number}</p><small>Recorded {formatDate(payment.receivedAt, { month: "short", day: "numeric", year: "numeric", hour: "numeric", minute: "2-digit" })}{paymentSettlementDate(payment) ? ` · settled ${formatDate(paymentSettlementDate(payment)!, { month: "short", day: "numeric", year: "numeric" })}` : " · not settled"}</small></div><StatusPill tone={paymentTone(payment.status)}>{payment.status}</StatusPill>{canFinance && payment.status === "Pending" && <Button size="sm" onClick={() => { setClearPaymentId(payment.id); setClearSettlementDate(todayKey()); }}>Record settlement</Button>}{canFinance && payment.status === "Cleared" && <Button size="sm" variant="ghost" onClick={() => { setRefundForm({ paymentId: payment.id, amount: String(payment.amount), reason: "", evidence: "" }); setRefundOpen(true); }}>Quality refund</Button>}</article>)}
