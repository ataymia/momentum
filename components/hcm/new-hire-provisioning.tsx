...
export function NewHireProvisioning({ view }: { view: NewHireView }) {
  ...
  useEffect(() => {
    ...
    const handle = window.setTimeout(() => {
      ...
      const started = linked && provisioning.beginOnboarding({ userId: user.id, draftId: draft.id });
      setNotice(started ? `${user.name} is ready for first sign-in and onboarding.` : "The Firebase account exists, but Momentum could not finish the onboarding link. Review the queue before trying again.");
      setPendingLink(null);
    }, 0);
  ...
  const createIdentity = async (draft: ProvisioningDraft) => {
    ...
    if (!result.ok || !result.uid) {
      setIdentityDraft({ ...identityDraft, busy: false, error: `${result.message ?? "Identity creation failed."} If Firebase created the identity but Firestore rejected access, do not repeatedly retry. Review Authentication before changing the password or recreating the account.` });
      return;
    }
  ...
  const startExistingOnboarding = (draft: ProvisioningDraft) => {
    ...
    const started = linked && provisioning.beginOnboarding({ userId: matchingUser.id, draftId: draft.id });
    setNotice(started ? `${matchingUser.name} is ready to begin onboarding.` : "The identity could not be linked to this onboarding setup. Review the account and draft before retrying.");
  };
