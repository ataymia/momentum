...
export function OnboardingPortal() {
  ...
  return <main className="onboarding-shell"><div className="onboarding-page">
    <header className="onboarding-header"><div><StatusPill tone="gold">New hire onboarding</StatusPill><h1>Welcome, {currentUser.firstName}</h1><p>Complete your required onboarding items below.</p></div><button className="onboarding-signout" onClick={logout}>Sign out</button></header>
    ...
    <div className="onboarding-grid">
      <section className="onboarding-panel"><header><UserRoundCheck size={19}/><div><h2>Your position</h2><p>Review your work setup.</p></div></header><dl><div><dt>Position</dt><dd>{employee?.jobTitle || currentUser.title}</dd></div><div><dt>Department</dt><dd>{employee?.department || currentUser.team}</dd></div><div><dt>Manager</dt><dd>{manager ? hcm.employees.find((item) => item.userId === manager.userId)?.jobTitle : "Configured in employee record"}</dd></div><div><dt>Work location</dt><dd>{employee?.location || "Not configured"}</dd></div><div><dt>Start date</dt><dd>{employee?.hireDate || "Not configured"}</dd></div></dl></section>

      <section className="onboarding-panel"><header><ShieldCheck size={19}/><div><h2>Contact & emergency profile</h2><p>Required before final activation.</p></div></header><div className="onboarding-check-list"><div className={profile?.phone ? "is-complete" : ""}><CheckCircle2 size={17}/><span>Phone number</span><small>{profile?.phone ? "Recorded" : "Not completed"}</small></div><div className={profile?.address ? "is-complete" : ""}><CheckCircle2 size={17}/><span>Home address</span><small>{profile?.address ? "Recorded" : "Not completed"}</small></div><div className={profile?.emergencyContact ? "is-complete" : ""}><CheckCircle2 size={17}/><span>Emergency contact</span><small>{profile?.emergencyContact ? "Recorded" : "Not completed"}</small></div></div><div className="onboarding-integration-gate"><LockKeyhole size={17}/><p>Secure profile entry will activate with the authenticated employee data connection.</p></div></section>
    ...
    <section className="onboarding-submit"><div><strong>Ready for review</strong><p>Submit when every required item is complete.</p></div><Button disabled={!readiness.readyForEmployeeSubmission} onClick={() => provisioning.submitOnboarding()}>Submit onboarding</Button></section>
  </div></main>;
}
