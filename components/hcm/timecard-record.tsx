...
export function TimecardRecord({timecardId,history=true}:{timecardId:string;history?:boolean}){
  ...
  const correctionRows=corrected.flatMap((entry)=>(entry.corrections??[]).map((correction,index)=>({entry,correction,index})));
