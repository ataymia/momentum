...
const movementTypes: MovementType[] = ["Receipt", "Transfer", "Allocation", "Release", "Delivery", "Return", "Sample", "Damage", "Shrink", "Adjustment", "Disposal"];
...
export function InventoryLedgerPanel() {
  ...
  const { ledger, postMovement, reserve, releaseReservation, fulfillReservation, recordCount, reconcileCount } = useInventoryLedger();
  ...
  return (
    <div className="page inventory-ledger-panel">
      <Section title="Inventory custody ledger" description="Every case movement is a source record. Warehouse, employee custody, customer delivery, returns, samples, damage, shrink, and disposal remain traceable.">
        <div className="company-rule-facts"><div><span>Ledger quantity</span><strong>{systemTotal} cs</strong><small>{selectedLot?.lotCode ?? "Select lot"}</small></div><div><span>Warehouse available</span><strong>{selectedLot ? warehouseAvailable(ledger, selectedLot.id) : 0} cs</strong><small>Warehouse balance less active reservations</small></div><div><span>Reserved</span><strong>{selectedLot ? reservedQuantity(ledger, selectedLot.id) : 0} cs</strong><small>Active order reservations</small></div><div><span>Open count exceptions</span><strong>{ledger.counts.filter((count) => count.status === "Open" && count.variance !== 0).length}</strong><small>Requires reconciliation reason</small></div></div>
      </Section>
      ...
      <div className="company-grid company-grid--two">
        <Section title="Movement history" description="Append-only custody transactions">
        ...
        <Section title="Order reservations" description="Reservation protects stock without pretending it physically moved">
          <div className="company-request-list">
            {reservations.map((item) => {
              ...
              return <article key={item.id}><span><PackageCheck size={17} /></span><div><small>{order?.number} · {item.status}</small><strong>{item.quantity} cases · {customer?.name ?? "Customer"}</strong><p>{location ? locationLabel(location) : "Location"}</p></div><StatusPill tone={item.status === "Active" ? "warning" : item.status === "Fulfilled" ? "success" : "neutral"}>{item.status}</StatusPill>{item.status === "Active" && <div className="request-actions"><Button size="sm" onClick={() => fulfillReservation(item.id)}>Fulfilled</Button><Button size="sm" variant="ghost" onClick={() => releaseReservation(item.id)}>Release</Button></div>}</article>;
            })}
      ...
      <Section title="Physical count & variance control" description="A count freezes system quantity and observed quantity. Variances require a reason before adjustment.">
        <div className="company-request-list">
          {counts.map((count) => {
            const node = ledger.nodes.find((item) => item.id === count.nodeId);
            return <article key={count.id}><span><ClipboardCheck size={17} /></span><div><small>{formatDate(count.countedAt, { month: "short", day: "numeric", hour: "numeric", minute: "2-digit" })} · {node?.name}</small><strong>Counted {count.countedQty} · system {count.systemQty} · variance {count.variance > 0 ? "+" : ""}{count.variance}</strong><p>{count.reason ?? "No reconciliation reason yet"}</p></div><StatusPill tone={countTone(count.status)}>{count.status}</StatusPill>{count.status === "Open" && <Button size="sm" onClick={() => { const reason = window.prompt("Explain the variance or confirm the count reconciliation"); if (reason) reconcileCount(count.id, reason); }}>Reconcile</Button>}</article>;
          })}
          {counts.length === 0 && <div className="review-empty"><p>No physical counts recorded for this lot.</p></div>}
        </div>
      </Section>

      <Modal open={movementOpen} title="Record inventory movement" description="Every movement requires quantity, custody path, reason, actor, and timestamp." onClose={() => setMovementOpen(false)} footer={<><Button variant="ghost" onClick={() => setMovementOpen(false)}>Cancel</Button><Button type="submit" form="movement-form">Post movement</Button></>} wide>
        <form id="movement-form" className="form-grid" onSubmit={submitMovement}><Field label="Movement type"><select value={movement.type} onChange={(event) => setMovement({ ...movement, type: event.target.value as MovementType })}>{movementTypes.map((type) => <option key={type}>{type}</option>)}</select></Field><Field label="Quantity (cases)"><input required type="number" min="0.01" step="0.01" value={movement.quantity} onChange={(event) => setMovement({ ...movement, quantity: event.target.value })} /></Field><Field label="From"><select value={movement.fromNodeId} onChange={(event) => setMovement({ ...movement, fromNodeId: event.target.value })}><option value="">No source / external adjustment</option>{ledger.nodes.filter((node) => node.active).map((node) => <option key={node.id} value={node.id}>{node.name} · {node.type}</option>)}</select></Field><Field label="To"><select value={movement.toNodeId} onChange={(event) => setMovement({ ...movement, toNodeId: event.target.value })}><option value="">No destination / remove from active custody</option>{ledger.nodes.filter((node) => node.active).map((node) => <option key={node.id} value={node.id}>{node.name} · {node.type}</option>)}</select></Field><Field label="Related order"><select value={movement.relatedOrderId} onChange={(event) => setMovement({ ...movement, relatedOrderId: event.target.value })}><option value="">None</option>{data.orders.map((order) => <option key={order.id} value={order.id}>{order.number} · {order.cases} cases</option>)}</select></Field><Field label="Reason" className="field--full"><textarea required rows={3} value={movement.reason} onChange={(event) => setMovement({ ...movement, reason: event.target.value })} /></Field></form>
      </Modal>

      <Modal open={countOpen} title="Record physical count" description="Count one lot at one custody point." onClose={() => setCountOpen(false)} footer={<><Button variant="ghost" onClick={() => setCountOpen(false)}>Cancel</Button><Button type="submit" form="count-form">Record count</Button></>}>
      ...
      <Modal open={reservationOpen} title="Reserve inventory for order" description="Reservations protect quantity before physical fulfillment." onClose={() => setReservationOpen(false)} footer={<><Button variant="ghost" onClick={() => setReservationOpen(false)}>Cancel</Button><Button type="submit" form="reservation-form">Reserve</Button></>}>
