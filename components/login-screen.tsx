    ...
    <main className="login-page">
      <section className="login-hero">
        ...
        <div className="login-hero__top">
          <BrandMark />
          <span className="demo-chip">Interactive V1 demo</span>
        </div>
        ...
        <div className="login-hero__content">
          ...
          <div className="operating-chain" aria-label="Operating workflow">
            {[
              ["01", "Sell", "Accounts & pipeline"],
              ["02", "Execute", "Dispatch & placement"],
              ["03", "Deliver", "Orders & inventory"],
              ["04", "Control", "Approvals & payroll"],
            ].map(([number, title, description]) => (
              <div className="operating-chain__item" key={number}>
                <span>{number}</span>
                <div>
                  <strong>{title}</strong>
        ...
        <footer className="login-hero__footer">
          <ShieldCheck size={17} />
          <span>Local demo data only · External services are not connected</span>
        </footer>
      </section>
      ...
      <section className="login-panel">
        <div className="login-panel__inner">
          <div className="login-panel__heading">
            <span className="login-panel__icon"><LockKeyhole size={20} /></span>
            <div>
              <p>Momentum Operations</p>
              <h2>Welcome back</h2>
            </div>
          </div>
          <p className="login-panel__intro">
            Sign in to tour the connected operating workspace.
          </p>
          ...
          <div className="demo-access">
            <div className="demo-access__title">
              <span>Tour another role</span>
            ...
            <div className="demo-access__users">
              {data.users.map((user) => (
                <button
                  ...
                  className={email === user.email ? "is-selected" : ""}
                >
                  <span style={{ background: user.accent }}>{user.initials}</span>
                  <div>
          ...
          <p className="login-disclaimer">
            This tour uses fictional contacts and clearly marked sample records. No live customer,
            employee, inventory, or financial data is present.
          </p>
        </div>
      </section>
    </main>
