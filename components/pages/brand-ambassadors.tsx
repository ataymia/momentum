...
export function BrandAmbassadorsPage() {
  ...
  return <div className="page page--focused-tool">
    ...
    {currentUser?.role==="Brand Ambassador"&&<Section title="Training" description="Training assigned by Momentum. Open the linked videos and materials for each module." action={<StatusPill tone="neutral"><GraduationCap size={14}/> {myTraining.length} module{myTraining.length===1?"":"s"}</StatusPill>}><div className="provisioning-queue">{myTraining.map(({assignment,course})=>{const materials=trainingLibrary.materialsForCourse(course!.id);return <article key={assignment.id}><span className="provisioning-avatar"><GraduationCap size={18}/></span><div><strong>{course!.title}</strong><p>{course!.description}</p>{materials.map((item)=><small key={item.id}><a href={item.url} target="_blank" rel="noreferrer">{item.title}</a> · {item.kind}</small>)}</div><StatusPill tone={assignment.status==="Complete"?"success":"warning"}>{assignment.status}</StatusPill></article>;})}{myTraining.length===0&&<div className="review-empty"><CheckCircle2 size={24}/><h3>No training assigned</h3><p>Your assigned modules will appear here.</p></div>}</div></Section>}
