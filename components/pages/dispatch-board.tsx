...
const tone = (status: AppointmentStatus) => status === "Completed" ? "success" as const : ["Dispatched", "En route", "Arrived"].includes(status) ? "info" as const : status === "Needs follow-up" ? "warning" as const : "neutral" as const;
...
type AppointmentForm = {
  ...
  priority: NonNullable<Appointment["priority"]>;
...
export function DispatchPage() {
  ...
  const [appointmentForm, setAppointmentForm] = useState<AppointmentForm>({
    ...
    ownerId: currentUser?.role === "Sales Representative" ? currentUser.id : "",
    date: selectedDate,
    startTime: "10:00",
    duration: 30,
    type: operationsMode ? "Delivery" : "First visit",
    priority: "Normal",
    arrivalWindow: "",
    objective: "",
    tags: "",
  ...
  const changeCustomer = (customerId: string) => {
    ...
    setAppointmentForm((current) => ({ ...current, customerId, accountId: locations[0]?.id ?? "" }));
  };

  const submitAppointment = (event: FormEvent) => {
    event.preventDefault();
    const id = createAppointment({
      accountId: appointmentForm.accountId,
      ownerId: appointmentForm.ownerId || undefined,
      date: appointmentForm.date,
      startTime: appointmentForm.startTime,
      duration: appointmentForm.duration,
      type: appointmentForm.type,
      objective: appointmentForm.objective,
      priority: appointmentForm.priority,
      arrivalWindow: appointmentForm.arrivalWindow || undefined,
      tags: appointmentForm.tags.split(",").map((item) => item.trim()).filter(Boolean),
    });
    if (!id) {
      setError("Choose a customer location in your scope and complete all required fields.");
      return;
    }
    setSelectedId(id);
  ...
  const openContext = (event: MouseEvent, appointmentId: string) => {
    if (!canManage) return;
    event.preventDefault();
    event.stopPropagation();
    setContextMenu({ appointmentId, x: event.clientX, y: event.clientY });
  };
  const dropOnTechnician = (event: DragEvent<HTMLDivElement>, ownerId: string) => {
    event.preventDefault();
    if (!dragId || !canManage) return;
    const rect = event.currentTarget.getBoundingClientRect();
    const relative = clamp(event.clientX - rect.left, 0, rect.width);
    const rawMinutes = BOARD_START + (relative / rect.width) * BOARD_SPAN;
    const snapped = clamp(Math.round(rawMinutes / SNAP) * SNAP, BOARD_START, BOARD_END - SNAP);
    moveAppointment(dragId, ownerId, selectedDate, minutesToTime(snapped));
    setDragId(null);
  };
  const dropInHolding = (event: DragEvent<HTMLDivElement>) => {
    event.preventDefault();
    if (!dragId || !canManage) return;
    reassignAppointment(dragId, "");
    setDragId(null);
  };
  const cardPosition = (appointment: Appointment) => {
    const start = clamp(timeToMinutes(appointment.startTime), BOARD_START, BOARD_END);
    return {
      left: `${((start - BOARD_START) / BOARD_SPAN) * 100}%`,
      width: `${Math.max(3, (appointment.duration / BOARD_SPAN) * 100)}%`,
    };
  };
  const openLocationRecord = () => {
    if (!selectedLocation) return;
    sessionStorage.setItem("momentum-focus-record", selectedLocation.id);
    navigate("accounts");
  };

  const appointmentCard = (appointment: Appointment) => {
    const location = scope.accounts.find((item) => item.id === appointment.accountId);
    ...
    return (
      <button
        ...
        onContextMenu={(event) => openContext(event, appointment.id)}
        onClick={() => setSelectedId(appointment.id)}
        className={`dispatch-time-card dispatch-time-card--${appointment.status.toLowerCase().replaceAll(" ", "-")} ${selected?.id === appointment.id ? "is-selected" : ""} ${draggable ? "is-draggable" : ""}`}
        style={cardPosition(appointment)}
        title={`${customer.name} · ${locationLabel(location)} · ${appointment.startTime}`}
      >
        {draggable && <GripVertical size={12} />}
  ...
  return (
    <div className="page page--dispatch dispatch-v2">
      <PageHeader
        eyebrow="Field operations"
        title="Dispatch board"
        description="Schedule the exact customer location and job without accidentally transferring an entire customer relationship."
        actions={canCreate ? <Button variant="gold" icon={<Plus size={17} />} onClick={() => setCreateOpen(true)}>{operationsMode ? "Schedule delivery" : "New appointment"}</Button> : undefined}
      />
      ...
      <div className="dispatch-toolbar-v2">
        <div className="date-tabs dispatch-date-tabs">
          {dates.map((date, index) => <button key={date} className={selectedDate === date ? "is-active" : ""} onClick={() => setSelectedDate(date)}><span>{index === 0 ? "Today" : formatDate(date, { weekday: "short" })}</span><strong>{formatDate(date, { month: "short", day: "numeric" })}</strong><i>{scope.appointments.filter((item) => item.date === date).length}</i></button>)}
        </div>
        <div className="dispatch-filters">
          <label>Field employee<select value={technicianFilter} onChange={(event) => setTechnicianFilter(event.target.value)}><option value="all">All</option>{assignable.map((user) => <option key={user.id} value={user.id}>{user.name}</option>)}</select></label>
          <label>Status<select value={statusFilter} onChange={(event) => setStatusFilter(event.target.value as typeof statusFilter)}><option value="all">All</option><option value="scheduled">Scheduled</option><option value="active">In motion</option><option value="closed">Closed</option></select></label>
        </div>
      </div>

      <section className="dispatch-timeline-shell">
      ...
      <section className="dispatch-holding" onDragOver={(event) => { if (canManage) event.preventDefault(); }} onDrop={dropInHolding}>
        <header><div><strong>Unassigned work</strong><span>{unassigned.length} appointment{unassigned.length === 1 ? "" : "s"} waiting for assignment</span></div><StatusPill tone={unassigned.length ? "warning" : "success"}>{unassigned.length ? "Needs dispatch" : "Clear"}</StatusPill></header>
        <div className="dispatch-holding-grid">
          {unassigned.map((appointment) => {
            const location = scope.accounts.find((item) => item.id === appointment.accountId);
            const customer = location ? customerForLocation(data, location) : undefined;
            return <button type="button" draggable={canManage && assignmentAllowed(appointment)} onDragStart={() => setDragId(appointment.id)} onDragEnd={() => setDragId(null)} onContextMenu={(event) => openContext(event, appointment.id)} onClick={() => setSelectedId(appointment.id)} key={appointment.id} className="dispatch-holding-card"><GripVertical size={15} /><div><small>{appointment.startTime} · {appointment.duration}m</small><strong>{customer?.name ?? "Customer"}</strong><span>{location ? locationLabel(location) : "Location"} · {appointment.type}</span></div></button>;
          })}
          {unassigned.length === 0 && <div className="dispatch-board-empty"><CheckCircle2 size={22} /><strong>Everything is assigned</strong></div>}
        </div>
      </section>
      ...
      <Modal open={createOpen} title={operationsMode ? "Schedule delivery" : "Schedule field work"} description="Choose the parent customer first, then the exact operating location." onClose={() => setCreateOpen(false)} footer={<><Button variant="ghost" onClick={() => setCreateOpen(false)}>Cancel</Button><Button type="submit" form="appointment-form">Schedule</Button></>} wide>
        <form id="appointment-form" className="form-grid" onSubmit={submitAppointment}>
          <Field label="Customer"><select required value={appointmentForm.customerId} onChange={(event) => changeCustomer(event.target.value)}>{visibleCustomers.map((customer) => <option key={customer.id} value={customer.id}>{customer.name}</option>)}</select></Field>
          <Field label="Location"><select required value={appointmentForm.accountId} onChange={(event) => setAppointmentForm({ ...appointmentForm, accountId: event.target.value })}>{formLocations.map((location) => <option key={location.id} value={location.id}>{locationLabel(location)}</option>)}</select></Field>
          <Field label="Assigned to"><select value={appointmentForm.ownerId} disabled={!canManage || currentUser?.role === "Sales Representative"} onChange={(event) => setAppointmentForm({ ...appointmentForm, ownerId: event.target.value })}><option value="">Unassigned</option>{assignable.map((user) => <option key={user.id} value={user.id}>{user.name}</option>)}</select></Field>
          ...
          <Field label="Duration"><input type="number" min={15} step={5} required value={appointmentForm.duration} onChange={(event) => setAppointmentForm({ ...appointmentForm, duration: Number(event.target.value) })} /></Field>
          <Field label="Work type"><select value={appointmentForm.type} onChange={(event) => setAppointmentForm({ ...appointmentForm, type: event.target.value as Appointment["type"] })}>{workTypes.map((type) => <option key={type}>{type}</option>)}</select></Field>
          <Field label="Priority"><select value={appointmentForm.priority} onChange={(event) => setAppointmentForm({ ...appointmentForm, priority: event.target.value as NonNullable<Appointment["priority"]> })}><option>Normal</option><option>High</option><option>Urgent</option></select></Field>
          <Field label="Customer arrival window"><input value={appointmentForm.arrivalWindow} onChange={(event) => setAppointmentForm({ ...appointmentForm, arrivalWindow: event.target.value })} placeholder="Optional, e.g. 10–11 AM" /></Field>
          <Field label="Tags"><input value={appointmentForm.tags} onChange={(event) => setAppointmentForm({ ...appointmentForm, tags: event.target.value })} placeholder="sample, cooler, chain" /></Field>
          <Field label="Objective" className="field--full"><textarea rows={3} required value={appointmentForm.objective} onChange={(event) => setAppointmentForm({ ...appointmentForm, objective: event.target.value })} /></Field>
      ...
      <Modal open={closeoutOpen} title="Close the work" description="Outcome, reflection, next action, and date are required." onClose={() => setCloseoutOpen(false)} footer={<><Button variant="ghost" onClick={() => setCloseoutOpen(false)}>Keep open</Button><Button type="submit" form="closeout-form">Complete work</Button></>}>
        <form id="closeout-form" className="form-grid" onSubmit={submitCloseout}>
          ...
          <Field label="Work note" className="field--full"><textarea rows={4} required value={closeout.closeoutNote} onChange={(event) => setCloseout({ ...closeout, closeoutNote: event.target.value })} /></Field>
