...
export function DispatchPage() {
  ...
  const focusAccount = scope.accounts.find(item => item.id === focus);
  ...
  const [createOpen,setCreateOpen] = useState(Boolean(focusAccount));
  const [closeoutOpen,setCloseoutOpen] = useState(false);
  const [error,setError] = useState("");
  const [appointmentForm,setAppointmentForm] = useState({ accountId:focusAccount?.id ?? scope.accounts[0]?.id ?? "", ownerId:currentUser?.id ?? "", date:selectedDate, startTime:"10:00", duration:30, type:(operationsMode ? "Delivery" : "First visit") as Appointment["type"], objective:"" });
  ...
  const submitAppointment = (event: FormEvent) => {
    ...
    const id = createAppointment(appointmentForm);
  ...
  const card = (appointment: Appointment) => {
    ...
    return <button className={`dispatch-card ${selected?.id === appointment.id ? "is-selected" : ""}`} key={appointment.id} onClick={() => setSelectedId(appointment.id)}><div className="dispatch-card__time"><Clock3 size={14}/><span>{appointment.startTime}</span><small>{appointment.duration}m</small></div><strong>{account?.name}</strong><p>{appointment.type}</p><div className="dispatch-card__location"><MapPin size={13}/> {appointment.location}</div><div className="dispatch-card__foot">{owner && <><Avatar initials={owner.initials} color={owner.accent} size="sm"/><span>{owner.firstName}</span></>}<StatusPill tone={tone(appointment.status)} dot={false}>{appointment.status}</StatusPill></div></button>;
  };

  return <div className="page page--dispatch">
    <PageHeader eyebrow="Field operations" title="Schedule & dispatch" description={operationsMode ? "Operations sees assigned logistics work and delivery work, not unrelated sales visits." : "Plan assigned work and close every visit with an outcome and dated next action."} actions={canCreate ? <Button variant="gold" icon={<Plus size={17}/>} onClick={() => setCreateOpen(true)}>{operationsMode ? "Schedule delivery" : "Schedule work"}</Button> : undefined}/>
    ...
    <div className="dispatch-layout"><div className="dispatch-board">{columns.map(column => { const items = appointments.filter(item => column.statuses.includes(item.status)); return <section className="dispatch-column" key={column.title}><header><div><h2>{column.title}</h2><p>{column.description}</p></div><span>{items.length}</span></header><div className="dispatch-column__cards">{items.map(card)}{items.length === 0 && <div className="dispatch-empty"><Route size={20}/><span>No work in this lane</span></div>}</div></section>; })}</div>
      <Section className="dispatch-detail" title="Work details" description="Operational result, separate from payroll time">{selected ? (() => { const account = scope.accounts.find(item => item.id === selected.accountId); const owner = data.users.find(item => item.id === selected.ownerId); return <div className="dispatch-detail__body"><div className="dispatch-detail__status"><StatusPill tone={tone(selected.status)}>{selected.status}</StatusPill><span>{selected.startTime} · {selected.duration} min</span></div><h3>{account?.name}</h3><p className="dispatch-detail__type">{selected.type}</p><div className="objective-card"><span>Work objective</span><p>{selected.objective}</p></div><dl className="detail-list"><div><dt><MapPin size={15}/> Location</dt><dd>{selected.location}</dd></div><div><dt><UserRoundCheck size={15}/> Assigned to</dt><dd>{owner?.name}</dd></div><div><dt><Clock3 size={15}/> Scheduled</dt><dd>{formatDate(selected.date,{month:"short",day:"numeric"})} at {selected.startTime}</dd></div></dl>{canManage && assignable.length > 0 && <label className="reassign-field"><span>Reassign assignment · change is recorded</span><select value={selected.ownerId} onChange={event => reassignAppointment(selected.id,event.target.value)}>{assignable.map(user => <option value={user.id} key={user.id}>{user.name} · {user.title}</option>)}</select></label>}{canOperate && !["Completed","Needs follow-up"].includes(selected.status) && <Button size="lg" icon={<ArrowRight size={17}/>} onClick={() => selected.status === "Arrived" ? setCloseoutOpen(true) : advanceAppointment(selected.id)}>{nextLabel[selected.status]}</Button>}{selected.status === "Completed" && <div className="completed-closeout"><div><CheckCircle2 size={17}/><strong>{selected.outcome ?? "Work completed"}</strong></div>{selected.closeoutNote && <p>{selected.closeoutNote}</p>}{selected.nextAction && <small>Next: {selected.nextAction} · {selected.nextActionDate ? formatDate(selected.nextActionDate) : "date not set"}</small>}</div>}</div>; })() : <div className="dispatch-empty"><p>Select an appointment.</p></div>}</Section>
    </div>

    <Modal open={createOpen} title={operationsMode ? "Schedule delivery" : "Schedule field work"} description={operationsMode ? "Operations can create delivery work; sales visit types remain outside this role." : "Create a dated assignment with one clear objective."} onClose={() => { setCreateOpen(false); sessionStorage.removeItem("momentum-focus-record"); }} footer={<><Button variant="ghost" onClick={() => setCreateOpen(false)}>Cancel</Button><Button type="submit" form="appointment-form">Schedule</Button></>}>
      <form id="appointment-form" className="form-grid" onSubmit={submitAppointment}><Field label="Account"><select required value={appointmentForm.accountId} onChange={event => setAppointmentForm({...appointmentForm,accountId:event.target.value})}>{scope.accounts.map(account => <option key={account.id} value={account.id}>{account.name}</option>)}</select></Field><Field label="Assigned to"><select value={appointmentForm.ownerId} disabled={!canManage || assignable.length <= 1} onChange={event => setAppointmentForm({...appointmentForm,ownerId:event.target.value})}>{assignable.map(user => <option key={user.id} value={user.id}>{user.name}</option>)}</select></Field><Field label="Date"><input type="date" required value={appointmentForm.date} onChange={event => setAppointmentForm({...appointmentForm,date:event.target.value})}/></Field><Field label="Start time"><input type="time" required value={appointmentForm.startTime} onChange={event => setAppointmentForm({...appointmentForm,startTime:event.target.value})}/></Field><Field label="Work type"><select value={appointmentForm.type} onChange={event => setAppointmentForm({...appointmentForm,type:event.target.value as Appointment["type"]})}>{workTypes.map(type => <option key={type}>{type}</option>)}</select></Field><Field label="Duration (minutes)"><input type="number" min={15} step={5} required value={appointmentForm.duration} onChange={event => setAppointmentForm({...appointmentForm,duration:Number(event.target.value)})}/></Field><Field label="Objective" className="field--full"><textarea rows={3} required value={appointmentForm.objective} onChange={event => setAppointmentForm({...appointmentForm,objective:event.target.value})} placeholder="What must be accomplished before this work can be closed?"/></Field>{error && <p className="form-error field--full" role="alert">{error}</p>}</form>
    </Modal>
    <Modal open={closeoutOpen} title="Close the work" description="An outcome and dated next action are required so work cannot disappear between stages." onClose={() => setCloseoutOpen(false)} footer={<><Button variant="ghost" onClick={() => setCloseoutOpen(false)}>Keep open</Button><Button type="submit" form="closeout-form">Complete work</Button></>}>
