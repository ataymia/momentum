...
import { openReceivables } from "../../lib/commerce-engine";
...
export function FinancePage() {
  ...
  const focusedExpense = focusId ? finance.expenses.find((expense) => expense.id === focusId) : undefined;
  ...
  const submittedExpenseCount = visibleExpenses.filter((item) => item.status === "Submitted").length;
  const payable = visibleExpenses.filter((expense) => expense.status === "Finance approved").reduce((sum, expense) => sum + expense.amount, 0);
  ...
  const recordReimbursementPaid = (expenseId: string) => {
    const reference = window.prompt("Enter the ACH, check, wire, or other payment reference used to reimburse this expense.");
    if (!reference?.trim()) return;
    markExpensePaid(expenseId, reference);
  };
