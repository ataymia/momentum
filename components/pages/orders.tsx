...
import { pricingTierPrice } from "../../lib/account-health";
...
export function OrdersPage() {
  ...
  const initialAccount = scope.accounts.find((account) => account.id === initialAccountId);
  ...
  const initialAccountPrice = pricingTierPrice(initialAccount?.pricingTier) ?? initialPricing?.currentPricePerCase ?? 0;
  ...
  const [form, setForm] = useState({ accountId:initialAccountId, product:initialProduct, cases:10, pricePerCase:initialAccountPrice });
  ...
  const formAccount = scope.accounts.find((account) => account.id === form.accountId);
  const formPricing = form.accountId ? evaluatePartnerPricing(data, form.accountId) : null;
  const directTierPrice = pricingTierPrice(formAccount?.pricingTier);
  const effectiveAccountPrice = directTierPrice ?? formPricing?.currentPricePerCase;
  const accountPriceLocked = Boolean(effectiveAccountPrice);
  ...
  const submit = (event: FormEvent) => {
    ...
    if (!customerMode && (!form.pricePerCase || form.pricePerCase <= 0) && !effectiveAccountPrice) { setFormError("Assign an A/B/C pricing tier on the account or enter an authorized price before submitting."); return; }
    const id = createOrder({ accountId:form.accountId, product:form.product, cases:form.cases, pricePerCase:effectiveAccountPrice ?? form.pricePerCase, inventoryAvailableAtOrder:stock?.available });
  ...
  const createCorrectedReplacement = () => {
    ...
    const account = scope.accounts.find((item) => item.id === selected.accountId);
    const pricing = evaluatePartnerPricing(data, selected.accountId);
    const accountPrice = pricingTierPrice(account?.pricingTier) ?? pricing.currentPricePerCase ?? selected.pricePerCase;
    ...
    const id = createOrder({ accountId:selected.accountId, product, cases:selected.cases, pricePerCase:accountPrice, inventoryAvailableAtOrder:available });
  ...
  const openDraft = (order?: typeof selected) => {
    ...
    const accountPrice = pricingTierPrice(account?.pricingTier) ?? pricing?.currentPricePerCase ?? order?.pricePerCase ?? 0;
    const product = order?.product ?? products[0] ?? "Golden Eagle";
    setForm({ accountId, product, cases:order?.cases ?? 10, pricePerCase:accountPrice });
  ...
  const changeOrderAccount = (accountId: string) => {
    ...
    const accountPrice = pricingTierPrice(account?.pricingTier) ?? pricing.currentPricePerCase ?? 0;
    setForm(current => ({ ...current, accountId, pricePerCase:accountPrice }));
    setFormError("");
  };
