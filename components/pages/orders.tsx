...
export function OrdersPage() {
  ...
  const intentProduct = typeof window !== "undefined" ? window.sessionStorage.getItem("momentum-order-product") : null;
  const sourcePlacementId = typeof window !== "undefined" ? window.sessionStorage.getItem("momentum-order-source-placement") : null;
  ...
  const initialProduct = intentProduct && products.includes(intentProduct) ? intentProduct : focusedOrder?.product ?? products[0] ?? "Golden Eagle";
  ...
  const sourcePlacement = sourcePlacementId ? scope.placements.find((placement)=>placement.id===sourcePlacementId&&placement.accountId===form.accountId&&placement.product===form.product) : undefined;

  useEffect(() => {
    if (typeof window !== "undefined") {
      window.sessionStorage.removeItem("momentum-focus-record");
      window.sessionStorage.removeItem("momentum-order-intent");
      window.sessionStorage.removeItem("momentum-order-product");
      window.sessionStorage.removeItem("momentum-order-source-placement");
    }
  }, [focusId, intent, intentProduct, sourcePlacementId]);
  ...
  const selectedStock = !customerMode && selected?.product ? productInventoryStatus(ledger,data,selected.product) : null;
  const selectedSourcePlacement = !customerMode && selected?.sourcePlacementId ? scope.placements.find((placement)=>placement.id===selected.sourcePlacementId) : undefined;
  const creditedRep = !customerMode && selected ? data.users.find((user) => user.id === (selected.creditedRepId ?? selected.ownerId) && user.role === "Sales Representative") : undefined;
  ...
  const reservedForSelected=!customerMode&&selected?activeReservedForOrder(ledger,selected.id):0;
  const outboundForSelected=!customerMode&&selected?orderOutboundQuantity(ledger,selected.id):0;
  const deliveredForSelected=selected?orderDeliveryQuantity(ledger,selected.id):0;
  const inventoryReady=Boolean(!customerMode&&selected&&nextStatus&&orderCanAdvanceInventory(ledger,selected,nextStatus));
  ...
  const submit = (event: FormEvent) => {
    ...
    if (!effectiveAccountPrice) {
      setFormError(customerMode ? "This account does not currently have an orderable price. Please contact your Momentum representative." : "This account does not currently have an orderable price. Partner Pricing is inactive and an authorized Tier B or Tier C fallback must be assigned before an order can be submitted.");
      return;
    }
    const id = createOrder({ accountId:form.accountId, product:form.product, cases:form.cases, inventoryAvailableAtOrder:stock?.available, sourcePlacementId:sourcePlacement?.id });
    if (!id) { setFormError(customerMode ? "The order could not be submitted. Please review the requested quantity or contact your Momentum representative." : "The order could not be created from the account's current pricing, inventory, or role permissions. Review the account and try again."); return; }
  ...
  const createCorrectedReplacement = () => {
    ...
    if (!pricing.currentPricePerCase) { setFormError(customerMode ? "This account does not currently have an orderable price. Please contact your Momentum representative." : "The account needs an authorized current pricing tier before this returned order can be replaced."); return; }
    ...
    const id = createOrder({ accountId:selected.accountId, product, cases:selected.cases, inventoryAvailableAtOrder:available, sourcePlacementId:selected.sourcePlacementId });
    if (!id) { setFormError(customerMode ? "The corrected order could not be submitted. Please contact your Momentum representative." : "This returned order could not be resubmitted from the current role."); return; }
    setSelectedId(id);
  };
  ...
  const openSourcePlacement=()=>{
    if(!selectedSourcePlacement)return;
    window.sessionStorage.setItem("momentum-focus-record",selectedSourcePlacement.id);
    navigate("retail");
  };
  ...
  return <div className="page page--orders">
    ...
    <div className="orders-layout"><Section className="orders-list" title="Order register"><label className="table-search order-search"><Search size={17} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search order, account, or product…" /></label><div className="order-table order-table--head"><span>Order</span><span>Customer</span><span>Cases</span><span>Amount</span><span>Status</span><span /></div><div className="order-table-body">{orders.map((order) => { const account = scope.accounts.find((item) => item.id === order.accountId); return <button className={`order-table ${selected?.id === order.id ? "is-selected" : ""}`} key={order.id} onClick={() => setSelectedId(order.id)}><span><strong>{order.number}</strong><small>{order.product ?? "Golden Eagle"}</small></span><span><strong>{account?.name}</strong><small>{account?.locationName ?? account?.location}</small></span><span>{order.cases}</span><span>{formatMoney(order.amount)}</span><span><StatusPill tone={toneForOrder(order.status)}>{order.status}</StatusPill></span><ChevronRight size={16} /></button>; })}</div></Section>
      {selected && <Section className="order-detail" title={selected.number} action={<StatusPill tone={toneForOrder(selected.status)}>{selected.status}</StatusPill>}><div className="order-detail__body"><div className="order-price-card"><div><span>Order total</span><strong>{formatMoney(selected.amount)}</strong><small>{selected.cases} cases × {formatMoney(selected.pricePerCase)}</small></div><span><FileText size={24} /></span></div><div className="order-detail-actions">{canCreateOrder(currentUser)&&<Button size="sm" variant="secondary" icon={<Copy size={14}/>} onClick={()=>openDraft(selected)}>Copy / reorder</Button>}{selectedSourcePlacement&&<Button size="sm" variant="ghost" icon={<Store size={14}/>} onClick={openSourcePlacement}>Open source placement</Button>}</div><div className="order-lifecycle" aria-label="Order lifecycle">{lifecycle.map((status,index) => <div className={index <= currentIndex ? "is-complete" : ""} key={status}><span>{index < currentIndex ? <CheckCircle2 size={14} /> : index + 1}</span><small>{status}</small></div>)}</div><dl className="order-facts"><div><dt>Product</dt><dd>{selected.product ?? "Golden Eagle"}</dd></div>{customerMode?<><div><dt>Price per case</dt><dd>{formatMoney(selected.pricePerCase)}</dd></div><div><dt>Payment</dt><dd>{selected.paymentStatus}</dd></div><div><dt>Delivered</dt><dd>{Math.min(deliveredForSelected,selected.cases)}/{selected.cases} cases</dd></div></>:<><div><dt>Pricing at order</dt><dd>{selected.priceBasis}</dd></div><div><dt>Sales credit</dt><dd>{creditedRep?.name ?? "No sales rep credit"}</dd></div><div><dt>Order source</dt><dd>{selectedSourcePlacement?`Retail placement · ${selectedSourcePlacement.id}`:"Direct order entry"}</dd></div><div><dt>Payment</dt><dd>{selected.paymentStatus}</dd></div><div><dt>Reserved</dt><dd>{reservedForSelected}/{selected.cases} cases</dd></div><div><dt>Outbound custody</dt><dd>{outboundForSelected}/{selected.cases} cases</dd></div><div><dt>Delivery evidence</dt><dd>{deliveredForSelected}/{selected.cases} cases</dd></div><div><dt>Available now</dt><dd>{selectedStock?.available ?? "Not tracked"}</dd></div></>}</dl>{!customerMode&&selected.lowStockApprovalRequired&&<div className="order-stock-gate"><ShieldAlert size={20}/><div><strong>Low-stock manager approval</strong><p>This request was created while the product had fewer than 50 available sellable cases. It cannot proceed to fulfillment until an authorized manager approves the order.</p></div></div>}{!customerMode&&selectedStock?.reorderNeeded&&<div className="order-stock-gate order-stock-gate--reorder"><PackageSearch size={20}/><div><strong>Warehouse reorder flag</strong><p>{selectedStock.available} available sellable cases remain. Management and warehouse should replenish below 500 cases.</p></div></div>}{selected.status === "Awaiting approval" ? <div className="order-approval-callout"><Store size={20} /><div><strong>{customerMode?"Order under review":canReview?"Approval required":"Order is under review"}</strong><p>{customerMode?"Your order request was received and is being reviewed before fulfillment.":selected.lowStockApprovalRequired?"The reviewer must verify low-stock availability before authorizing fulfillment.":"An authorized reviewer must approve the order before fulfillment begins."}</p></div>{canReview && <Button size="sm" onClick={() => navigate("work")}>Review</Button>}</div> : selected.status === "Draft" ? <div className="order-approval-callout"><AlertCircle size={20}/><div><strong>Returned for correction</strong><p>{customerMode?"Review the order details and submit a corrected request.":"The original request remains in history."}</p></div>{canCorrect && <Button size="sm" onClick={createCorrectedReplacement}>Create corrected replacement</Button>}</div> : nextStatus && canFulfill && inventoryReady ? <Button size="lg" icon={<ArrowRight size={17} />} onClick={advanceFulfillment}>Move to {nextStatus.toLowerCase()}</Button> : nextStatus && canFulfill ? <div className="order-approval-callout"><PackageSearch size={20}/><div><strong>Inventory evidence required before {nextStatus.toLowerCase()}</strong><p>{inventoryRequirement}. Open Inventory to complete the missing reservation or custody record.</p></div><Button size="sm" variant="secondary" onClick={()=>navigate("inventory")}>Open inventory</Button></div> : nextStatus && customerMode ? <div className="order-approval-callout"><Truck size={20}/><div><strong>Order in progress</strong><p>Momentum is processing this order. The status above will update as it moves toward delivery.</p></div></div> : nextStatus ? <div className="order-approval-callout"><Truck size={20}/><div><strong>Fulfillment in progress</strong><p>Authorized Operations staff control order status. Warehouse records the physical custody evidence that unlocks each transition.</p></div></div> : selected.status === "Delivered" ? <div className="order-approval-callout"><CircleDollarSign size={20}/><div><strong>Delivery complete · payment pending</strong><p>Delivery does not mark the order paid.</p></div></div> : <div className="order-complete"><CheckCircle2 size={20} /><span>Paid record locked</span></div>}{formError && <p className="form-error" role="alert">{formError}</p>}</div></Section>}
    </div>
    <Modal open={createOpen} title={customerMode ? "Place order" : "Create order request"} onClose={() => { setCreateOpen(false); setFormError(""); }} footer={<><Button variant="ghost" onClick={() => setCreateOpen(false)}>Cancel</Button><Button type="submit" form="new-order-form" disabled={!effectiveAccountPrice}>Submit</Button></>}><form id="new-order-form" className="form-grid" onSubmit={submit}><Field label={customerMode ? "Your account" : "Customer account"}><select value={form.accountId} onChange={(event) => changeOrderAccount(event.target.value)}>{scope.accounts.map((account) => <option value={account.id} key={account.id}>{account.name} · {account.locationName ?? account.location}</option>)}</select></Field><Field label="Product"><select value={form.product} onChange={(event)=>setForm({...form,product:event.target.value})}>{products.map((product)=><option key={product}>{product}</option>)}</select></Field><Field label="Cases"><input type="number" min="1" required value={form.cases} onChange={(event) => setForm({ ...form, cases:Number(event.target.value) })} /></Field>{!customerMode && <Field label="Current price per case"><input readOnly value={effectiveAccountPrice ? formatMoney(effectiveAccountPrice) : "Pricing setup required"} /></Field>}{sourcePlacement&&!customerMode&&<div className="form-callout field--full"><Store size={17}/><p>Reorder started from placement {sourcePlacement.id}. This order will retain that placement as its source record.</p></div>}<div className="partner-price-order-card"><Store size={18}/><div><small>{customerMode?"Current order price":"Pricing"}</small><strong>{customerMode?(effectiveAccountPrice?`${formatMoney(effectiveAccountPrice)} per case`:"Price unavailable"):formPricing?.effectiveTier?`Tier ${formPricing.effectiveTier} · ${formPricing.status}`:"Outside-Partner tier required"}</strong>{!customerMode&&<p>{effectiveAccountPrice ? `${formatMoney(effectiveAccountPrice)} per case. ${formPricing?.reason ?? ""}` : formPricing?.reason ?? "A valid account pricing state is required before ordering."}</p>}</div>{effectiveAccountPrice&&<StatusPill tone={customerMode?"success":pricingTone(formPricing?.status ?? "")}>{formatMoney(effectiveAccountPrice)}</StatusPill>}</div>{stock&&!customerMode&&<div className={`order-inventory-check ${stock.requiresManagerApproval?"is-critical":stock.reorderNeeded?"is-reorder":""}`}><Box size={19}/><div><span>Available sellable inventory</span><strong>{stock.available} cases</strong><small>{stock.requiresManagerApproval?"Manager approval required before this sale can proceed.":stock.reorderNeeded?"Warehouse reorder flag is active.":"Stock level does not trigger a threshold."}</small></div></div>}<div className="order-preview"><Box size={20} /><div><span>{customerMode ? "Requested quantity" : "Request total"}</span><strong>{customerMode ? `${form.cases} cases` : effectiveAccountPrice ? formatMoney(form.cases * effectiveAccountPrice) : "Pricing setup required"}</strong></div></div>{!effectiveAccountPrice&&<div className="form-callout form-callout--warning"><AlertCircle size={17}/><p>{customerMode?"Current pricing is not available for online ordering. Please contact your Momentum representative.":"Partner Pricing is not active and no authorized Tier B or Tier C fallback is assigned. An administrator or sales manager must resolve account pricing before this order can be submitted."}</p></div>}{formError && <p className="form-error field--full" role="alert">{formError}</p>}{stock?.requiresManagerApproval&&!customerMode&&<div className="form-callout form-callout--warning"><ShieldAlert size={17}/><p>Submitting creates an approval request. Do not represent this quantity as committed until management approves the low-stock sale.</p></div>}</form></Modal>
  </div>;
}