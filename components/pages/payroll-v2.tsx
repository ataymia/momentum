...
export function PayrollPage() {
  ...
  const recordLiabilityPaid = (id: string) => {
    const reference = window.prompt("Payment or deposit confirmation reference");
    if (!reference?.trim()) return;
    const paidDate = window.prompt("Payment date (YYYY-MM-DD)", today());
    if (!paidDate) return;
    setNotice(updateLiability(id, "Paid", { reference, date: paidDate }) ? "Tax payment evidence recorded" : "Tax payment could not be recorded. Confirm the liability is scheduled, the reference is present, and the date is not in the future.");
  };
  const recordDisbursementSettlement = (id: string) => {
    const reference = window.prompt("Bank or payment-rail settlement reference");
    if (!reference?.trim()) return;
    const settlementDate = window.prompt("Settlement date (YYYY-MM-DD)", today());
    if (!settlementDate) return;
    setNotice(settleDisbursement(id, reference, settlementDate) ? "Employee payment settlement recorded" : "Settlement could not be recorded. Confirm the payment is released, the reference is present, and the date is not in the future.");
  };
  const recordDisbursementFailure = (id: string) => {
    const reason = window.prompt("Failure or return reason");
    if (!reason?.trim()) return;
    setNotice(failDisbursement(id, reason) ? "Payment failure recorded" : "Payment failure could not be recorded");
  };
  const retryFailedDisbursement = (id: string) => {
    const replacementId = retryDisbursement(id);
    setNotice(replacementId ? `Replacement payment instruction ${replacementId} released` : "Payment could not be retried. Confirm payroll is still valid and the employee payment destination is configured.");
  };
  ...
  return <div className="page page--payroll">
    ...
    {tab === "tax" && admin && <div className="company-grid company-grid--two"><Section title="Tax liability ledger" description="Scheduling a payment is an operational action. Marking it paid requires external payment evidence and records the fact even if the underlying payroll later needs correction."><div className="tax-ledger">{payroll.liabilities.map((liability) => { const record=taxLiabilityRecord(liability); return <article key={liability.id}><div><small>{liability.type} · {liability.payRunId}</small><strong>{formatMoney(liability.amount)}</strong>{record.paymentReference&&<p>Paid {record.paidAt} · ref {record.paymentReference}</p>}</div><StatusPill tone={liability.status === "Paid" ? "success" : liability.status === "Reversed" ? "danger" : "warning"}>{liability.status}</StatusPill>{liability.status === "Accrued" && <Button size="sm" onClick={() => setNotice(updateLiability(liability.id,"Scheduled") ? "Tax liability scheduled" : "Liability cannot be scheduled while the source payroll is invalid or locked")}>Schedule</Button>}{liability.status === "Scheduled" && <Button size="sm" variant="gold" onClick={() => recordLiabilityPaid(liability.id)}>Record paid</Button>}</article>; })}{payroll.liabilities.length === 0 && <div className="review-empty"><p>No tax liabilities posted.</p></div>}</div></Section><Section title="Disbursement register" description="Released instructions can be reconciled as settled or failed from external evidence. Failed payments may be retried only while the source payroll remains valid."><div className="tax-ledger">{payroll.disbursements.map((item) => { const record=disbursementRecord(item); const evidence=record.settlementReference?`Settled ${record.settledAt} · ref ${record.settlementReference}`:record.failureReason?`Failed: ${record.failureReason}${record.replacedBy?` · replacement ${record.replacedBy}`:""}`:record.retryOf?`Retry of ${record.retryOf}`:item.tokenLabel||"No masked destination"; return <article key={item.id}><div><small>{data.users.find((user)=>user.id===item.userId)?.name} · {item.method} · {item.payRunId}</small><strong>{formatMoney(item.amount)}</strong><p>{evidence}</p></div><StatusPill tone={item.status === "Settled" ? "success" : item.status === "Voided" || item.status === "Failed" ? "danger" : "warning"}>{item.status}</StatusPill>{item.status === "Released" && <div className="request-actions"><Button size="sm" onClick={() => recordDisbursementSettlement(item.id)}>Record settled</Button><Button size="sm" variant="danger" onClick={() => recordDisbursementFailure(item.id)}>Record failure</Button></div>}{item.status === "Failed" && !record.replacedBy && <Button size="sm" variant="gold" onClick={() => retryFailedDisbursement(item.id)}>Retry instruction</Button>}</article>; })}{payroll.disbursements.length === 0 && <div className="review-empty"><p>No payment instructions released.</p></div>}</div></Section></div>}
