...
import { BadgeDollarSign, Calculator, Check, CircleDollarSign, FileText, LockKeyhole, Play, ShieldCheck, WalletCards } from "lucide-react";
...
type PayLine = { employeeId:string; regularHours:number; regularPay:number; bonusPay:number; grossPay:number; taxWithholding:number; deductions:number; netPay:number };
type PayRun = { id:string; createdAt:string; periodStart:string; periodEnd:string; status:"Draft"|"Approved"|"Released"; lines:PayLine[] };
...
export function PayrollPage() {
  ...
  const myRuns = state.runs.filter((run) => run.lines.some((line) => line.employeeId === currentUser?.id));
  ...
  const calculateLine = (employeeId:string, periodStart:string, periodEnd:string):PayLine | null => {
    ...
    const bonusPay = earnedBonuses.filter((signal) => signal.repId === employeeId).reduce((sum,signal) => sum + signal.amount,0);
    ...
    return { employeeId,regularHours:hours,regularPay,bonusPay,grossPay,taxWithholding,deductions,netPay:Math.max(0,grossPay-taxWithholding-deductions) };
  };
  const buildDraft = () => {
    if (!admin) return;
    const readyCards = data.timecards.filter((card) => card.status === "Payroll ready" || card.status === "Manager approved");
    if (readyCards.length === 0) return;
    const periodStart = readyCards.map((card) => card.weekStart).sort()[0]; const periodEnd = readyCards.map((card) => card.weekEnd).sort().at(-1)!;
    const employeeIds = [...new Set(readyCards.map((card) => card.userId))];
    const lines = employeeIds.map((id) => calculateLine(id,periodStart,periodEnd)).filter((line): line is PayLine => Boolean(line));
    if (lines.length === 0) return;
    setState((current) => ({ ...current,runs:[{ id:`pay-${Date.now()}`,createdAt:new Date().toISOString(),periodStart,periodEnd,status:"Draft",lines },...current.runs] }));
  };
  const setRunStatus = (id:string,status:"Approved"|"Released") => { if (!admin) return; setState((current) => ({ ...current,runs:current.runs.map((run) => run.id === id ? { ...run,status } : run) })); };
  ...
  return <div className="page page--payroll"><PageHeader eyebrow="Workforce compensation" title="Payroll" description="Momentum calculates earnings from approved source records, preserves every pay-line component, and creates the payment-release instruction without surrendering the payroll workflow to another platform." actions={<StatusPill tone="gold">Native payroll engine</StatusPill>}/>
    ...
    {tab === "self" && <div className="company-grid company-grid--two"><Section title="Payroll profile" description="The employee can see the pay basis and current configuration that affects their statement"><div className="payroll-profile"><div><span><BadgeDollarSign size={20}/></span><div><small>Pay basis</small><strong>{myConfig ? `${myConfig.payType} · ${formatMoney(myConfig.rate)}` : "Not configured"}</strong></div></div><div><span><Calculator size={20}/></span><div><small>Withholding setup</small><strong>{myConfig ? `${myConfig.taxPercent}% tax · ${myConfig.deductionPercent}% + ${formatMoney(myConfig.flatDeduction)} deductions` : "Not configured"}</strong></div></div><div><span><WalletCards size={20}/></span><div><small>Payment method</small><strong>Payment rail not connected</strong></div></div></div></Section><Section title="Latest statement" description="Every net-pay number must drill into hours, earnings, bonuses, withholding, and deductions">{latestMyLine ? <div className="pay-statement"><div><span>Regular pay</span><strong>{formatMoney(latestMyLine.regularPay)}</strong></div><div><span>Bonus earnings</span><strong>{formatMoney(latestMyLine.bonusPay)}</strong></div><div><span>Gross pay</span><strong>{formatMoney(latestMyLine.grossPay)}</strong></div><div><span>Tax withholding</span><strong>-{formatMoney(latestMyLine.taxWithholding)}</strong></div><div><span>Deductions</span><strong>-{formatMoney(latestMyLine.deductions)}</strong></div><div className="pay-statement__net"><span>Net pay</span><strong>{formatMoney(latestMyLine.netPay)}</strong></div></div> : <div className="review-empty"><FileText size={24}/><h3>No pay statement yet</h3><p>A statement appears after an authorized payroll run includes this employee.</p></div>}</Section></div>}

    {tab === "processing" && admin && <><div className="payroll-control-alert"><ShieldCheck size={20}/><div><strong>Payroll is built from approved records, not manual memory.</strong><p>Timecards feed regular pay. Earned sales bonuses feed variable pay. Effective-dated payroll configuration supplies rates, withholding, and deductions. Release remains a separate approval.</p></div><Button icon={<Play size={15}/>} onClick={buildDraft}>Build draft run</Button></div><Section title="Pay-run register" description="Draft → approved → released; each employee line remains recalculable from source records"><div className="campaign-register">{state.runs.map((run) => <article key={run.id}><div className="campaign-register__head"><div><small>{formatDate(run.periodStart,{month:"short",day:"numeric"})} – {formatDate(run.periodEnd,{month:"short",day:"numeric",year:"numeric"})}</small><strong>{run.id}</strong><p>{run.lines.length} employee line{run.lines.length === 1 ? "" : "s"} · gross {formatMoney(run.lines.reduce((sum,line) => sum + line.grossPay,0))} · net {formatMoney(run.lines.reduce((sum,line) => sum + line.netPay,0))}</p></div><StatusPill tone={run.status === "Released" ? "success" : run.status === "Approved" ? "gold" : "info"}>{run.status}</StatusPill></div><div className="finance-order-list"><div className="finance-order-row finance-order-row--head"><span>Employee</span><span>Hours</span><span>Regular</span><span>Bonus</span><span>Net</span></div>{run.lines.map((line) => { const employee=data.users.find((user) => user.id === line.employeeId); return <div className="finance-order-row" key={line.employeeId}><span>{employee?.name}</span><span>{line.regularHours.toFixed(2)}</span><span>{formatMoney(line.regularPay)}</span><span>{formatMoney(line.bonusPay)}</span><span><strong>{formatMoney(line.netPay)}</strong></span></div>; })}</div>{run.status === "Draft" && <Button size="sm" icon={<Check size={14}/>} onClick={() => setRunStatus(run.id,"Approved")}>Approve payroll</Button>}{run.status === "Approved" && <Button size="sm" variant="gold" icon={<WalletCards size={14}/>} onClick={() => setRunStatus(run.id,"Released")}>Release payment instruction</Button>}</article>)}{state.runs.length === 0 && <div className="review-empty"><p>No payroll runs have been built.</p></div>}</div></Section></>}
