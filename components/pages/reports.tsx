...
import { localDateKey } from "../../lib/management-kpi";
...
export function ReportsPage() {
  ...
  const metrics = reportScope === "commercial"
    ? [
    ...
    : reportScope === "operations"
      ? [
          ["On-hand inventory", `${onHand} cs`, "Formula: sum recorded lot onHand"],
          ["Available inventory", `${available} cs`, "Formula: sum lot available; holds/reservations excluded by source record"],
  ...
  return <div className="page page--reports">
    ...
    {reportScope === "commercial" && <Section title="Commercial record flow" description="Each milestone is reported separately" className="reality-panel">
    ...
    {reportScope === "people" && <Section title="Human Resources metric controls" description="Current formulas and privacy boundary">
      <div className="accounting-rule-list">
        <article><span><ShieldCheck size={17}/></span><div><strong>Scope</strong><p>Administrators see company workforce records. Managers see only employees inside their permitted management scope.</p></div></article>
