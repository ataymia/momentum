...
export function RetailPage() {
  ...
  const selectPlacement = (placementId: string) => {
    ...
    setCheck({
      ...
      cold: placement.cold,
      shelfPrice: placement.shelfPrice,
    });
  };
  ...
  return (
    <div className="page page--retail">
      ...
      <div className="commercial-proof-chain">
        {[
          [PackageCheck, "Sell-in", "Order recorded", scope.orders.length],
          [Store, "Placement", "Available to buy", scope.placements.length],
          [Eye, "Observation", "Stock checked", scope.placements.filter((item) => item.lastChecked).length],
          [RefreshCcw, "Reorder", "Customer bought again", scope.accounts.filter((item) => item.reorderCount > 0).length],
      ...
      <div className="retail-kpis">
        <div><span><Store size={19} /></span><div><small>Healthy placements</small><strong>{healthy} / {scope.placements.length}</strong></div></div>
        <div><span><Snowflake size={19} /></span><div><small>Cold availability</small><strong>{cold} / {scope.placements.length}</strong></div></div>
      ...
      <div className="retail-layout">
        <Section title="Active placements" description="Demo records by store location" className="placement-list-panel">
          <div className="placement-grid">
            {scope.placements.map((placement) => {
          ...
          return (
            <Section title="Placement check" description={account?.name} className="placement-detail" action={<StatusPill tone="gold">Demo</StatusPill>}>
              ...
              <form className="placement-check-form" onSubmit={submit}>
                ...
                <Field label="Observed shelf price"><input type="number" min="0" step="0.01" value={check.shelfPrice} onChange={(event) => setCheck({ ...check, shelfPrice: Number(event.target.value) })} /></Field>
              ...
              <button className="reorder-link" onClick={() => { window.sessionStorage.setItem("momentum-focus-record", selected.accountId); navigate("orders"); }}><RefreshCcw size={17} /><span>Move to reorder workflow</span><ChevronRight size={16} /></button>
            </Section>
          );
        })()}
      </div>
