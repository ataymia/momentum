...
export function RetailPage() {
  ...
  const [selectedId, setSelectedId] = useState(scope.placements[0]?.id ?? "");
  const selected = scope.placements.find((placement) => placement.id === selectedId) ?? scope.placements[0];
  const [check, setCheck] = useState({ observedStock:selected?.observedStock ?? 0, facings:selected?.facings ?? 0, cold:selected?.cold ?? false, shelfPrice:selected?.shelfPrice ?? 0 });
  ...
  const submit = (event: FormEvent) => { event.preventDefault(); if (selected) updatePlacement(selected.id, check.observedStock, check.facings, check.cold, check.shelfPrice); };
  ...
  return <div className="page page--retail">
    ...
    <div className="retail-layout"><Section title="Active placements" description="Demo records by store location" className="placement-list-panel"><div className="placement-grid">{scope.placements.map((placement) => { const account = scope.accounts.find((item) => item.id === placement.accountId); return <button key={placement.id} onClick={() => selectPlacement(placement.id)} className={`placement-card ${selected?.id === placement.id ? "is-selected" : ""}`}><div className="placement-card__top"><span><Store size={18} /></span><StatusPill tone={placement.status === "Healthy" ? "success" : placement.status === "Out of stock" ? "danger" : "warning"}>{placement.status}</StatusPill></div><h3>{account?.name}</h3><p>{placement.location}</p><div className="placement-card__stats"><div><small>Observed units</small><strong>{placement.observedStock}</strong></div><div><small>Facings</small><strong>{placement.facings}</strong></div><div><small>Cold</small><strong>{placement.cold ? "Yes" : "No"}</strong></div></div><footer><span>Checked {formatDate(placement.lastChecked, { month:"short", day:"numeric" })}</span><ChevronRight size={16} /></footer></button>; })}</div></Section>
      {selected && (() => { const account = scope.accounts.find((item) => item.id === selected.accountId); return <Section title="Placement check" description={account?.name} className="placement-detail" action={<StatusPill tone="gold">Demo</StatusPill>}><div className="placement-detail__product"><span className="product-can" aria-hidden="true"><i>GE</i></span><div><small>Product record</small><strong>{selected.product}</strong><p>{selected.location} · ${selected.shelfPrice.toFixed(2)} observed shelf price</p></div></div><div className="movement-estimate"><div><small>Cases delivered</small><strong>{selected.casesDelivered}</strong></div><div><small>Observed stock</small><strong>{selected.observedStock} units</strong></div><div><small>Evidence source</small><strong>{selected.source}</strong></div></div><p className="movement-caveat"><CircleAlert size={15} /> No unit conversion or sell-through estimate is calculated until the product case configuration is approved.</p><form className="placement-check-form" onSubmit={submit}><Field label="Observed units"><input type="number" min="0" value={check.observedStock} onChange={(event) => setCheck({ ...check, observedStock:Number(event.target.value) })} /></Field><Field label="Facings"><input type="number" min="0" value={check.facings} onChange={(event) => setCheck({ ...check, facings:Number(event.target.value) })} /></Field><Field label="Observed shelf price"><input type="number" min="0" step="0.01" value={check.shelfPrice} onChange={(event) => setCheck({ ...check, shelfPrice:Number(event.target.value) })} /></Field><label className="toggle-field"><span><strong>Cold availability</strong><small>Product available chilled</small></span><input type="checkbox" checked={check.cold} onChange={(event) => setCheck({ ...check, cold:event.target.checked })} /><i /></label><div className="evidence-row"><span><Camera size={17} /> Photo evidence</span><small>Unavailable until Firebase Storage is connected</small></div><Button type="submit" icon={<Check size={17} />}>Save observation</Button></form><button className="reorder-link" onClick={beginReorder}><RefreshCcw size={17} /><span>Start reorder</span><ChevronRight size={16} /></button></Section>; })()}
    </div>
  </div>;
}
