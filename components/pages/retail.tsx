...
export function RetailPage() {
  ...
  const focusedPlacement = focusId ? scope.placements.find((placement) => placement.id === focusId) : undefined;
  const [selectedId, setSelectedId] = useState(focusedPlacement?.id ?? "");
  const [detailOpen, setDetailOpen] = useState(Boolean(focusedPlacement));
  const selected = scope.placements.find((placement) => placement.id === selectedId);
  const [check, setCheck] = useState(focusedPlacement ? { observedStock:focusedPlacement.observedStock, facings:focusedPlacement.facings, cold:focusedPlacement.cold, shelfPrice:focusedPlacement.shelfPrice } : { observedStock:0, facings:0, cold:false, shelfPrice:0 });

  useEffect(() => {
    if (focusedPlacement && focusId) window.sessionStorage.removeItem("momentum-focus-record");
  }, [focusId, focusedPlacement]);
