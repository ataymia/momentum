...
export function StatusPill({ children, tone = "neutral", dot = true }: { children: ReactNode; tone?: "neutral" | "success" | "warning" | "danger" | "info" | "gold"; dot?: boolean; }) {
  return <span className={`status-pill status-pill--${tone}`}>{dot && <i aria-hidden="true" />}{children}</span>;
}

export function MetricCard({ label, value, detail, trend, tone = "blue", onClick }: { label: string; value: string; detail: string; trend?: string; tone?: "blue" | "gold" | "red" | "green"; onClick?: () => void; }) {
  const Tag = onClick ? "button" : "article";
  return <Tag className={`metric-card metric-card--${tone}`} onClick={onClick}><div className="metric-card__top"><span>{label}</span>{onClick && <ArrowUpRight size={16} aria-hidden="true" />}</div><strong>{value}</strong><div className="metric-card__bottom"><span>{detail}</span>{trend && <b>{trend}</b>}</div></Tag>;
}

export function Button({ children, variant = "primary", size = "md", icon, ...props }: React.ButtonHTMLAttributes<HTMLButtonElement> & { variant?: "primary" | "secondary" | "ghost" | "danger" | "gold"; size?: "sm" | "md" | "lg"; icon?: ReactNode; }) {
  return <button className={`button button--${variant} button--${size}`} {...props}>{icon}<span>{children}</span></button>;
}

export function TextButton({ children, onClick }: { children: ReactNode; onClick?: () => void }) {
  return <button className="text-button" onClick={onClick}><span>{children}</span><ChevronRight size={15} aria-hidden="true" /></button>;
}
