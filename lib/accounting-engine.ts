...
export function buildPayrollVoidJournal(state:AccountingState,run:PayRun,actorId:string):JournalEntry|null{
  ...
  const entry:JournalEntry={id:`journal-payroll-void-${run.id}`,number,date:date(run.voidedAt),memo:`Payroll correction reversal · ${run.id} · ${run.voidReason??"voided payroll run"}`,status:"Draft",sourceType:"Payroll voided",sourceId:run.id,lines,createdAt:now(),createdBy:actorId};
  return journalBalanced(entry)?entry:null;
}
