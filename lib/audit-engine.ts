...
export function collectAuditableRecords(module: string, state: unknown): Map<string, AuditSnapshot> {
  ...
  for (const [collection, value] of Object.entries(state as Record<string, unknown>)) {
    ...
    for (const item of value) {
      if (!item || typeof item !== "object") continue; const record = item as Record<string, unknown>; const id = text(record.id); if (!id) continue;
      const entityType = `${module}.${collection}`; const key = `${entityType}:${id}`;
      records.set(key, { key, module, collection, entityType, entityId: id, label: recordLabel(record, id), sensitivity: sensitivityFor(module, collection), relatedAccountId: relatedAccount(record, module, collection, id), relatedUserId: relatedUser(record, module, collection, id), payload: record });
    }
  }
  return records;
}
...
export function diffAuditableRecords(previous: Map<string, AuditSnapshot>, current: Map<string, AuditSnapshot>, actor: { id: string; role: string }, at = new Date().toISOString()): AuditEvent[] {
  const events: AuditEvent[] = []; const keys = new Set([...previous.keys(), ...current.keys()]); let sequence = 0;
  for (const key of keys) { const before = previous.get(key); const after = current.get(key); if (before && after) { let same = false; try { same = JSON.stringify(before.payload) === JSON.stringify(after.payload); } catch { same = false; } if (same) continue; }
    const snapshot = after ?? before; if (!snapshot) continue; const action: AuditEvent["action"] = !before ? "Created" : !after ? "Deleted" : "Updated"; const changes = changeList(before?.payload, after?.payload); const changedFields = changes.map((item) => item.field).join(", ");
    events.push({ id: `audit-${Date.now()}-${sequence++}-${Math.random().toString(36).slice(2, 6)}`, at, actorId: actor.id, actorRole: actor.role, action, module: snapshot.module, collection: snapshot.collection, entityType: snapshot.entityType, entityId: snapshot.entityId, label: snapshot.label, summary: action === "Updated" ? `${snapshot.label} updated${changedFields ? `: ${changedFields}` : ""}` : `${snapshot.label} ${action.toLowerCase()}`, sensitivity: snapshot.sensitivity, relatedAccountId: after?.relatedAccountId ?? before?.relatedAccountId, relatedUserId: after?.relatedUserId ?? before?.relatedUserId, changes });
  }
  return events;
}
...
export function visibleAuditEvents(user: WorkspaceUser | null, data: WorkspaceData, events: AuditEvent[]) {
  ...
  const accountIds = new Set(data.accounts.filter((account) => user.role === "Sales Manager" ? managedIds.has(account.ownerId) : user.role === "Sales Representative" ? account.ownerId === user.id : true).map((account) => account.id));
  return events.filter((event) => {
    if (user.role === "Sales Manager") return event.sensitivity !== "admin" && ((!event.relatedAccountId && !event.relatedUserId) || (event.relatedAccountId ? accountIds.has(event.relatedAccountId) : false) || (event.relatedUserId ? managedIds.has(event.relatedUserId) : false));
    if (user.role === "Sales Representative") return event.sensitivity === "operational" && ((event.relatedAccountId ? accountIds.has(event.relatedAccountId) : false) || event.relatedUserId === user.id);
    if (user.role === "Operations") return event.sensitivity === "operational" && (event.module === "Inventory" || ["orders", "appointments", "inventory"].includes(event.collection) || event.relatedUserId === user.id);
    return false;
  });
}
