...
const now=()=>new Date().toISOString();const today=()=>arizonaDateKey();const validDateKey=(value?:string)=>Boolean(value&&/^\d{4}-\d{2}-\d{2}$/.test(value));
const businessDateKey=(value:string)=>validDateKey(value)?value:arizonaDateKey(value);
...
export function paymentSettlementDate(payment:Payment){return payment.settledAt?businessDateKey(payment.settledAt):payment.status==="Cleared"?businessDateKey(payment.receivedAt):undefined;}
...
export function canRecordSettlementDate(value?:string){return Boolean(value&&validDateKey(value)&&value<=today());}
...
export function refundCanSettle(refund:Refund,settlementDate:string){if(refund.status!=="Sent"||!refund.sentAt||!canRecordSettlementDate(settlementDate))return false;return settlementDate>=businessDateKey(refund.sentAt);}
