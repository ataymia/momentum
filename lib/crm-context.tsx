...
export function CrmProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    const handle = window.setTimeout(() => {
      setCrm((state) => normalizeCrmState(state, data));
    }, 0);
    return () => window.clearTimeout(handle);
  }, [data]);

  useEffect(() => {
    if (typeof window !== "undefined") window.localStorage.setItem(CRM_STORAGE_KEY, JSON.stringify(crm));
  }, [crm]);
  ...
  const addContact = (input: NewContact) => {
    ...
    setCrm((state) => ({
      ...
      contacts: [
        record,
        ...(input.primary
          ? state.contacts.map((item) => item.scope === input.scope && item.customerId === input.customerId && item.locationId === input.locationId ? { ...item, primary: false } : item)
          : state.contacts),
      ],
    }));
    return id;
  ...
  const addInteraction = (input: NewInteraction) => {
    ...
    setCrm((state) => ({ ...state, interactions: [record, ...state.interactions] }));
    return id;
  };
  ...
  const addOpportunity = (input: NewOpportunity) => {
    ...
    setCrm((state) => ({ ...state, opportunities: [record, ...state.opportunities] }));
    return id;
  };

  const updateOpportunity = (id: string, patch: Partial<Opportunity>) => {
    setCrm((state) => ({ ...state, opportunities: state.opportunities.map((item) => item.id === id ? { ...item, ...patch, updatedAt: now() } : item) }));
  };

  const recordResponsibility = (input: Omit<ResponsibilityEvent, "id" | "effectiveAt" | "changedBy"> & { effectiveAt?: string }) => {
    const record: ResponsibilityEvent = { ...input, id: uid("responsibility"), effectiveAt: input.effectiveAt ?? now(), changedBy: currentUser?.id ?? "system" };
    setCrm((state) => ({ ...state, responsibilityHistory: [record, ...state.responsibilityHistory] }));
  };
