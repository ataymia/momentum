...
export function parseInventoryImport(rows: CsvRow[]): ImportResult<InventoryLot> {
  ...
  rows.forEach((row,index) => {
    ...
    if(reserved>onHand){errors.push(`Row ${rowNumber}: reserved cannot exceed onHand.`);return;}
    records.push({id:row.id||"",lotCode:row.lotCode,product:row.product,receivedAt,bestBy,onHand,reserved,available:onHand-reserved,status:row.status as InventoryLot["status"],location:row.location,holdReason:row.holdReason||undefined});
  });
  return{records,errors};
}
...
export type OrderImportRow={accountId:string;cases:number;pricePerCase?:number;product?:string};
export function parseOrderImport(rows:CsvRow[]):ImportResult<OrderImportRow>{const records:OrderImportRow[]=[];const errors:string[]=[];rows.forEach((row,index)=>{const rowNumber=index+2;if(!required(row,["accountId","cases"],rowNumber,errors))return;const cases=numberValue(row.cases,"cases",rowNumber,errors,{min:1,integer:true});const price=row.pricePerCase?numberValue(row.pricePerCase,"pricePerCase",rowNumber,errors,{min:0}):undefined;if(cases===undefined||row.pricePerCase&&price===undefined)return;records.push({accountId:row.accountId,cases,pricePerCase:price,product:row.product||undefined});});return{records,errors};}
