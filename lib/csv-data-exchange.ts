...
export function parseAppointmentImport(rows:CsvRow[]):ImportResult<AppointmentImportRow>{
  const records:AppointmentImportRow[]=[];const errors:string[]=[];const types=new Set<Appointment["type"]>(["First visit","Revisit","Sample drop","Placement check","Reorder","Delivery"]);const priorities=new Set(["Normal","High","Urgent"]);
