...
export function parseOrderImport(rows:CsvRow[]):ImportResult<OrderImportRow>{const records:OrderImportRow[]=[];const errors:string[]=[];rows.forEach((row,index)=>{const rowNumber=index+2;if(!required(row,["accountId","cases"],rowNumber,errors))return;const cases=numberValue(row.cases,"cases",rowNumber,errors,{min:1,integer:true});if(cases===undefined)return;records.push({accountId:row.accountId,cases,product:row.product||undefined});});return{records,errors};}
