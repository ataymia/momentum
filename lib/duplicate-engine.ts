...
export function findAccountDuplicate(accounts: Account[], input: AccountDuplicateInput): DuplicateMatch | null {
  const street = normalizeText(input.streetAddress); const name = normalizeText(input.name); const location = normalizeText(input.locationName || input.location); const phone = normalizePhone(input.phone); const email = normalizeEmail(input.email);
  for (const account of accounts) {
    const accountStreet = normalizeText(account.streetAddress); const accountName = normalizeText(account.name); const accountLocation = normalizeText(account.locationName || account.location); const accountPhone = normalizePhone(account.phone); const accountEmail = normalizeEmail(account.email);
    ...
    if (name && accountName === name && location && accountLocation === location) return { account, reason: "That business and location already exist.", confidence: "Exact" };
