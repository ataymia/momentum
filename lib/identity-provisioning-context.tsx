...
export function IdentityProvisioningProvider({ children }: { children: ReactNode }) {
  ...
  const beginOnboarding = (input: BeginOnboardingInput) => {
    ...
    if (!draft || target.email.toLowerCase() !== draft.workEmail.toLowerCase() || target.role !== draft.role || target.team !== draft.team || target.managerId !== draft.managerId) return false;
