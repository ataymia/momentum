...
export function MarketingProvider({children}:{children:ReactNode}){
  ...
  const createCampaign=(input:Omit<Campaign,"id"|"createdAt"|"status">&{status?:Campaign["status"]})=>{
    if(!isAdmin||!currentUser||!input.name.trim()||!input.objective.trim()||!input.audience.trim()||!input.startDate||!input.endDate||input.endDate<input.startDate||!Number.isFinite(input.requestedBudget)||input.requestedBudget<0||!input.successMeasure.trim())return"";
  ...
  const addTouch=(input:Omit<MarketingTouch,"id"|"createdBy">)=>{if(!isAdmin||!currentUser||!campaignExists(input.campaignId)||!accountInScope(input.accountId)||!input.occurredAt||!input.summary.trim())return"";const id=uid("marketing-touch");setState((s)=>({...s,touches:[{...input,summary:input.summary.trim(),id,createdBy:currentUser.id},...s.touches]}));return id;};
