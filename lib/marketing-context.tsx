...
export function MarketingProvider({children}:{children:ReactNode}){
  ...
  const submitRequest=(input:Omit<MarketingRequest,"id"|"submittedAt"|"status">)=>{
    ...
    if(input.neededBy!==undefined&&!isValidCalendarDateKey(input.neededBy))return"";
    if(input.materialItemId&&!state.materials.some((item)=>item.id===input.materialItemId&&item.active))return"";
    if(input.materialItemId&&input.quantity===undefined)return"";
  ...
  const recordSpend=(input:Omit<MarketingSpend,"id"|"submittedAt"|"status"|"submittedBy">)=>{
    if(!isAdmin||!currentUser)return null;const campaign=state.campaigns.find((item)=>item.id===input.campaignId);if(!campaign||!["Approved","Active","Complete"].includes(campaign.status)||!isValidCalendarDateKey(input.date)||!input.vendor.trim()||!input.category.trim()||!Number.isFinite(input.amount)||input.amount<=0||!input.businessPurpose.trim()||campaign.approvedBudget===undefined||!Number.isFinite(campaign.approvedBudget))return null;const committed=state.spend.filter((item)=>item.campaignId===campaign.id&&["Submitted","Approved","Reconciled"].includes(item.status)).reduce((sum,item)=>sum+(Number.isFinite(item.amount)?item.amount:0),0);if(committed+input.amount>campaign.approvedBudget)return null;const id=uid("marketing-spend");const record:MarketingSpend={...input,vendor:input.vendor.trim(),category:input.category.trim(),businessPurpose:input.businessPurpose.trim(),id,status:"Submitted",submittedBy:currentUser.id,submittedAt:now()};setState((s)=>({...s,spend:[record,...s.spend]}));return id;
  };
  ...
  const addAsset=(input:Omit<Asset,"id"|"updatedAt"|"updatedBy">)=>{if(!isAdmin||!input.name.trim()||!input.type.trim()||!Number.isInteger(input.version)||input.version<1||(input.effectiveDate!==undefined&&!isValidCalendarDateKey(input.effectiveDate)))return"";const id=uid("asset");const record:Asset={...input,name:input.name.trim(),type:input.type.trim(),id,updatedAt:now(),updatedBy:currentUser?.id??"system"};setState((s)=>({...s,assets:[record,...s.assets]}));return id;};
  ...
  const addMaterial=(input:Omit<MaterialItem,"id">)=>{if(!isAdmin||!input.name.trim()||!input.unit.trim()||!Number.isFinite(input.openingQty)||input.openingQty<0||!Number.isFinite(input.reorderPoint)||input.reorderPoint<0)return"";const id=uid("material");setState((s)=>({...s,materials:[{...input,name:input.name.trim(),unit:input.unit.trim(),id},...s.materials]}));return id;};
  const moveMaterial=(input:Omit<MaterialMovement,"id"|"at"|"actorId">)=>{if(!isAdmin||!Number.isFinite(input.quantity)||input.quantity<=0||!input.reason.trim()||!state.materials.some((item)=>item.id===input.itemId)||!accountInScope(input.accountId)||!campaignExists(input.campaignId))return null;if(input.type==="Issue"&&materialBalance(state,input.itemId)<input.quantity)return null;const id=uid("material-movement");const record:MaterialMovement={...input,reason:input.reason.trim(),id,at:now(),actorId:currentUser?.id??"system"};setState((s)=>({...s,materialMovements:[record,...s.materialMovements]}));return id;};
  const addTouch=(input:Omit<MarketingTouch,"id"|"createdBy">)=>{if(!isAdmin||!currentUser||!campaignExists(input.campaignId)||!accountInScope(input.accountId)||!isValidCalendarDateKey(input.occurredAt)||!input.summary.trim())return"";const id=uid("marketing-touch");setState((s)=>({...s,touches:[{...input,summary:input.summary.trim(),id,createdBy:currentUser.id},...s.touches]}));return id;};
  const addAttribution=(input:Omit<MarketingAttribution,"id"|"createdAt"|"createdBy"|"reviewed">)=>{if(!isAdmin||!currentUser||!campaignExists(input.campaignId)||!accountInScope(input.accountId)||!input.note.trim()||!sourceExists(input))return"";const id=uid("attribution");setState((s)=>({...s,attributions:[{...input,note:input.note.trim(),id,createdAt:now(),createdBy:currentUser.id,reviewed:false},...s.attributions]}));return id;};
  ...
  const addPartnership=(input:Omit<Partnership,"id"|"createdAt"|"ownerId">)=>{if(!isAdmin||!currentUser||!input.name.trim()||!input.type.trim()||!input.notes.trim()||!campaignExists(input.campaignId)||(input.startDate!==undefined&&!isValidCalendarDateKey(input.startDate))||(input.endDate!==undefined&&!isValidCalendarDateKey(input.endDate))||(input.startDate&&input.endDate&&input.endDate<input.startDate))return"";const id=uid("partnership");setState((s)=>({...s,partnerships:[{...input,name:input.name.trim(),type:input.type.trim(),notes:input.notes.trim(),id,createdAt:now(),ownerId:currentUser.id},...s.partnerships]}));return id;};
