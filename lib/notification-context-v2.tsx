...
export function NotificationProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    const handle = window.setTimeout(() => setState((current) => {
      ...
      return copyChanged || additions.length ? { ...current, deliveries: [...additions, ...refreshed].slice(0, 400) } : current;
    }), 0);
  ...
  useEffect(() => {
    const handle = window.setTimeout(() => setState((current) => {
      ...
      return additions.length || retained.length !== current.deliveries.length ? { ...current, deliveries: [...additions, ...retained].slice(0, 400) } : current;
    }), 0);
    return () => window.clearTimeout(handle);
  }, [data, ledger]);
