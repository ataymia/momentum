...
export function PeriodLockProvider({ children }: { children: ReactNode }) {
  ...
  const [state, setState] = useState<PeriodLockState>(() => readState());
  useEffect(() => { if (typeof window !== "undefined") window.localStorage.setItem(PERIOD_LOCK_STORAGE_KEY, JSON.stringify(state)); }, [state]);
  const createLock = (domain: PeriodLockDomain, startDate: string, endDate: string, reason: string) => {
    if (currentUser?.role !== "Administrator" || !startDate || !endDate || endDate < startDate || reason.trim().length < 4) return null;
    const id = uid(); const record: PeriodLock = { id, domain, startDate, endDate, reason: reason.trim(), lockedAt: new Date().toISOString(), lockedBy: currentUser.id };
    setState((current) => ({ ...current, locks: [record, ...current.locks] })); return id;
  };
  const releaseLock = (id: string, reason: string) => {
    if (currentUser?.role !== "Administrator" || reason.trim().length < 4) return false;
    let changed = false; setState((current) => ({ ...current, locks: current.locks.map((lock) => { if (lock.id !== id || lock.releasedAt) return lock; changed = true; return { ...lock, releasedAt: new Date().toISOString(), releasedBy: currentUser.id, releaseReason: reason.trim() }; }) })); return changed;
  };
