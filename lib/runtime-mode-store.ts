...
export const RUNTIME_MODE_SEED: RuntimeModeState = { version: 1, mode: "demo" };
...
export function readRuntimeModeState(): RuntimeModeState {
  ...
  try {
    ...
    return value?.mode === "production" || value?.mode === "demo"
      ? { version: 1, mode: value.mode, changedAt: value.changedAt, changedBy: value.changedBy }
      : RUNTIME_MODE_SEED;
  } catch {
...
export function writeRuntimeModeState(state: RuntimeModeState) {
  ...
  window.localStorage.setItem(RUNTIME_MODE_STORAGE_KEY, JSON.stringify(state));
  window.dispatchEvent(new CustomEvent(RUNTIME_MODE_EVENT, { detail: state }));
}
...
export function useRuntimeModeValue() {
  return useSyncExternalStore(subscribeRuntimeMode, currentRuntimeMode, () => "demo" as RuntimeMode);
}
