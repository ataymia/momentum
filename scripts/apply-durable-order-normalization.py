...
sales_rep_ids = '''  const salesRepIds = new Set(data.users.filter((user) => user.role === "Sales Representative").map((user) => user.id));\n'''
if sales_rep_ids in text:
    text = text.replace(sales_rep_ids, "", 1)
